...
void gasAt(vec3 p,float rd,float dt,out float dens,out vec3 tint,out float heat){
  ...
  float inner=smoothstep(0.0,0.07,rn),outer2=1.0-smoothstep(0.45,1.0,rn);
  float prof=inner*outer2*pow(uDiskIn/rd,2.0);
...
vec3 starField(vec3 d){
  vec3 a=abs(d); vec2 uv2; float face;
  if(a.x>=a.y&&a.x>=a.z){uv2=d.yz/a.x;face=d.x>0.0?0.0:1.0;}
  else if(a.y>=a.z){uv2=d.xz/a.y;face=d.y>0.0?2.0:3.0;}
  else{uv2=d.xy/a.z;face=d.z>0.0?4.0:5.0;}
  vec3 col=vec3(0.0);
  for(int k=0;k<3;k++){
    float sc=90.0*pow(2.2,float(k));
    vec2 p2=uv2*sc,id=floor(p2),f2=fract(p2)-0.5;
    float h2=hash13(vec3(id,face*19.0));
    if(h2>0.965){
      vec2 off=vec2(hash13(vec3(id,face+11.0)),hash13(vec3(id,face+23.0)));
      float dd=length(f2-(off-0.5)*0.7);
      float s=smoothstep(0.055,0.0,dd);
      float warm=hash13(vec3(id,face+51.0));
      col+=s*(0.6+4.5*fract(h2*97.0))*mix(vec3(0.72,0.82,1.0),vec3(1.0,0.88,0.72),warm)/pow(2.2,float(k));
    }
  }
...
void main(){
  vec2 uv3=(gl_FragCoord.xy+uJitter-uFocus*uRes)/uRes.y;
  vec3 dir=normalize(uFwd+(uv3.x*uRight+uv3.y*uUp)*2.0*uTanHalf);
  vec3 pos=uCamPos,vel=dir;
  vec3 hv=cross(pos,vel);float h2=dot(hv,hv),h=sqrt(h2),swept=0.0;
  vec3 col=vec3(0.0);float transmit=1.0;bool captured=false;
  float jitter2=fract(sin(dot(gl_FragCoord.xy+uSeed,vec2(12.9898,78.233)))*43758.5453);
  for(int i=0;i<MAX_STEPS;i++){
    ...
    if(r<uDiskOut*1.25){
      float rn2=clamp((r-uDiskIn)/max(0.001,uDiskOut-uDiskIn),0.0,1.0);
      float tk2=uThick*(0.35+1.25*rn2);
      dt=min(dt,max(tk2*0.38,abs(pos.y)*0.5));
    }
    swept+=h*dt/r2;
    float deep=exp(-1.3*max(0.0,swept-4.6));
    jitter2=fract(jitter2+0.6180339887);
    vec3 mid2=pos+vel*(dt*jitter2);
    float rd2=length(mid2.xz);
    if(rd2>uDiskIn&&rd2<uDiskOut&&abs(mid2.y)<uThick*5.0){
      float dens2;float heat2;vec3 tint2;
      gasAt(mid2,rd2,dt,dens2,tint2,heat2);
      if(dens2>0.001){
        vec3 tang=normalize(cross(vec3(0.0,1.0,0.0),vec3(mid2.x,0.0,mid2.z)));
        float beta=min(0.85,sqrt(0.5/max(rd2,1.5)));
        float gam=inversesqrt(max(1e-4,1.0-beta*beta));
        vec3 toObs=-normalize(vel);
        float g=1.0/(gam*(1.0-beta*dot(tang,toObs)));
        g*=sqrt(max(0.05,1.0-1.0/rd2));
        ...
        float emit=uBright*(0.26+2.0*heat2*heat2);
        col+=tint2*shift*(emit*boost*dens2*transmit*dt*deep);
        transmit*=exp(-dens2*0.30*dt);
      }
    }
...
void main(){
  vec3 c=texture2D(uCur,vUv).rgb,p=texture2D(uPrev,vUv).rgb;
...
void main(){
  ...
  vec2 dv=vUv-0.5;
  c*=1.0-uVignette*dot(dv,dv)*1.9;
  if(uScrimDir>0.5){
    float x2=uScrimDir<1.5?vUv.x:uScrimDir<2.5?1.0-vUv.x:uScrimDir<3.5?1.0-vUv.y:vUv.y;
    c*=1.0-uScrimAmt*pow(1.0-clamp(x2,0.0,1.0),2.4);
  }
...
export function BlackHoleHeroSection({
  ...
  const props = useRef({ distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter, diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor, midColor, coolColor, starBrightness, glow, exposure, vignette, steps, resolution, maxDpr, focus, scrim, scrimStrength, paused });
  props.current = { distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter, diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor, midColor, coolColor, starBrightness, glow, exposure, vignette, steps, resolution, maxDpr, focus, scrim, scrimStrength, paused };
  ...
  useEffect(() => {
    ...
    let hdr = true, texType: number = gl.UNSIGNED_BYTE, internal: number = gl.RGBA;
    if (isGL2) {
      ...
      const hf = gl.getExtension("OES_texture_half_float"), cb = gl.getExtension("EXT_color_buffer_half_float");
    ...
    function dropTargets() {
      ...
      scene = histA = histB = bloomA = bloomB = null; settled = 0;
    }
    function resize() {
      ...
      dropTargets();
      scene = makeTarget(sw, sh); histA = makeTarget(sw, sh); histB = makeTarget(sw, sh);
    ...
    function render(t: number) {
      ...
      const az = (C.azimuth + C.orbitSpeed * t) * RAD, el = Math.max(-88, Math.min(88, C.elevation)) * RAD;
      const dist = Math.max(2.2, C.distance), ce = Math.cos(el);
      const camX = dist * ce * Math.cos(az), camY = dist * Math.sin(el), camZ = dist * ce * Math.sin(az);
      const fx = -camX/dist, fy = -camY/dist, fz = -camZ/dist;
      let rx = fz, ry = 0, rz = -fx; const rl = Math.hypot(rx, ry, rz) || 1;
      rx /= rl; ry /= rl; rz /= rl;
      ...
      const hot = hexToLinear(C.hotColor), mid2 = hexToLinear(C.midColor), cool2 = hexToLinear(C.coolColor);
      const outer = Math.max(C.diskInner + 0.5, C.diskOuter);

      function pass(prog: Prog, target: Target | null) {
        gl!.useProgram(prog.program); gl!.bindFramebuffer(gl!.FRAMEBUFFER, target ? target.fb : null);
        gl!.viewport(0, 0, target ? target.w : width, target ? target.h : height);
      }
      function draw2() { gl!.drawArrays(gl!.TRIANGLES, 0, 3); }
      function bind2(tex: WebGLTexture, unit: number) { gl!.activeTexture(gl!.TEXTURE0 + unit); gl!.bindTexture(gl!.TEXTURE_2D, tex); }

      pass(sceneProg, scene);
      const u = sceneProg.u;
      gl!.uniform2f(u.uRes!, scene.w, scene.h); gl!.uniform1f(u.uTime!, t);
      gl!.uniform3f(u.uCamPos!, camX, camY, camZ); gl!.uniform3f(u.uRight!, RX, RY, RZ);
      gl!.uniform3f(u.uUp!, UX, UY, UZ); gl!.uniform3f(u.uFwd!, fx, fy, fz);
      ...
      gl!.uniform1f(u.uDiskIn!, Math.max(1.05, C.diskInner)); gl!.uniform1f(u.uDiskOut!, outer);
      gl!.uniform1f(u.uThick!, Math.max(0.02, C.diskThickness)); gl!.uniform1f(u.uDensity!, Math.max(0, C.diskDensity));
      gl!.uniform1f(u.uSpin!, C.spinSpeed * 6.2831853); gl!.uniform1f(u.uGrain!, Math.max(0.02, C.grain));
      gl!.uniform1f(u.uBright!, Math.max(0, C.brightness)); gl!.uniform1f(u.uDoppler!, Math.max(0, Math.min(1, C.doppler)));
      gl!.uniform3f(u.uHot!, hot[0], hot[1], hot[2]); gl!.uniform3f(u.uMid!, mid2[0], mid2[1], mid2[2]); gl!.uniform3f(u.uCool!, cool2[0], cool2[1], cool2[2]);
      gl!.uniform1f(u.uStars!, Math.max(0, C.starBrightness)); gl!.uniform1f(u.uEncode!, hdr ? 0 : 1);
      const hh = HALTON[settled % HALTON.length];
      gl!.uniform2f(u.uJitter!, hh[0]-0.5, hh[1]-0.5); gl!.uniform1f(u.uSeed!, (settled % 64) * 17.13);
      draw2();

      const alpha = settled === 0 ? 1 : 0.14;
      pass(blendProg, histB);
      bind2(scene.tex, 0); bind2(histA.tex, 1);
      gl!.uniform1i(blendProg.u.uCur!, 0); gl!.uniform1i(blendProg.u.uPrev!, 1); gl!.uniform1f(blendProg.u.uAlpha!, alpha);
      draw2();
      const shown = histB; histA = histB; histB = shown; settled++;

      pass(brightProg, bloomA);
      bind2(histA.tex, 0);
      gl!.uniform1i(brightProg.u.uTex!, 0); gl!.uniform2f(brightProg.u.uTexel!, 1/histA.w, 1/histA.h);
      gl!.uniform1f(brightProg.u.uDecode!, hdr ? 0 : 1); gl!.uniform1f(brightProg.u.uPack!, pack); gl!.uniform1f(brightProg.u.uThreshold!, 0.85);
      draw2();

      function blurStep(src: Target, dst: Target, dx: number, dy: number) {
        pass(blurProg!, dst); bind2(src.tex, 0);
        gl!.uniform1i(blurProg!.u.uTex!, 0); gl!.uniform2f(blurProg!.u.uStep!, dx/dst.w, dy/dst.h); draw2();
      }
      blurStep(bloomA, bloomB, 1, 0); blurStep(bloomB, bloomA, 0, 1);
      blurStep(bloomA, bloomB, 2.6, 0); blurStep(bloomB, bloomA, 0, 2.6);

      pass(compProg, null);
      bind2(histA.tex, 0); bind2(bloomA.tex, 1);
      gl!.uniform1i(compProg.u.uScene!, 0); gl!.uniform1i(compProg.u.uBloom!, 1);
      gl!.uniform2f(compProg.u.uRes!, width, height);
      gl!.uniform1f(compProg.u.uDecode!, hdr ? 0 : 1); gl!.uniform1f(compProg.u.uPack!, pack);
    ...
    let clock = reduced ? 6 : 0, lastFrame = 0, running = true, visible = true, raf = 0;
    ...
    resize(); settle(reduced ? 16 : 1);
    if (!reduced) raf = requestAnimationFrame(tick);

    const ro = new ResizeObserver(() => { resize(); if (reduced || props.current.paused) settle(16); });
    ro.observe(host);
    ...
    const onRestored = () => {
      ...
      canvas.style.display = ""; host.dataset.webgl = ""; resize(); running = true; lastFrame = 0;
      settle(reduced ? 16 : 1); if (!reduced) raf = requestAnimationFrame(tick);
    };
