      ...
      >
        ...
        {error && !loading && (
          <li className="px-2 py-2.5 text-[13px]">
            <p className="text-zinc-600 dark:text-zinc-300">Couldn&rsquo;t load options.</p>
            <button
              type="button"
              onClick={onRetry}
        ...
        {!loading && !error && filtered.length === 0 && (
          <li className="px-2 py-3 text-[13px] text-zinc-500 dark:text-zinc-400">No matches</li>
        )}
          ...
          return (
            <li
              ...
              className={[
                ...
                isActive ? "bg-zinc-100 dark:bg-white/10" : "bg-transparent",
            ...
            >
              ...
              {item.glyph && (
                <span className="shrink-0" aria-hidden>{item.glyph}</span>
              )}
              <span className="truncate">{item.label}</span>
              {!multi && item.selected && (
                <span className="ml-auto text-zinc-500 dark:text-zinc-400"><CheckIcon /></span>
              )}
            </li>
          );
        })}
  ...
  return (
    <button
      ...
      className={[
        ...
        "active:scale-[0.98]",
        muted ? "text-zinc-500 dark:text-zinc-400" : "text-zinc-800 dark:text-zinc-100",
        active ? "bg-zinc-200/70 dark:bg-white/[0.12]" : "hover:bg-zinc-200/60 dark:hover:bg-white/[0.08]",
...
export function FilterBar({
  ...
  const onToolbarKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowRight") { e.preventDefault(); focusItem(focusIdx >= last ? 0 : focusIdx + 1); }
    else if (e.key === "ArrowLeft") { e.preventDefault(); focusItem(focusIdx <= 0 ? last : focusIdx - 1); }
    else if (e.key === "Home") { e.preventDefault(); focusItem(0); }
    else if (e.key === "End") { e.preventDefault(); focusItem(last); }
    else if (e.key === "Backspace" || e.key === "Delete") {
      const meta = itemMeta[focusIdx];
      if (meta?.filterId) { e.preventDefault(); removeFilter(meta.filterId, focusIdx); }
    }
  };
  ...
  const addFilter = (fieldId: string) => {
    ...
    if (!field) return;
    const filter: Filter = { id: uid(), field: fieldId, operator: field.operators[0]?.value ?? "is", values: [] };
  ...
  const changeField = (filterId: string, fieldId: string) => {
    const field = fieldById(fields, fieldId);
    onChange(value.map((f) => f.id === filterId
      ? { ...f, field: fieldId, operator: field?.operators[0]?.value ?? f.operator, values: [] }
      : f
    ));
  ...
  const changeOperator = (filterId: string, opValue: string) => {
    ...
    onChange(value.map((f) => f.id === filterId
      ? { ...f, operator: opValue, values: nextOp?.multi ? f.values : f.values.slice(0, 1) }
      : f
    ));
    setOpen(null);
  };
  ...
  const openPopoverContent = () => {
    ...
    if (open.kind === "operator") {
      const items: ListItem[] = field.operators.map((o) => ({ value: o.value, label: o.label, selected: o.value === filter.operator }));
      return (
        <SearchList
          key={anchorKey}
          labelId="fb-op-label"
    ...
    const asyncData = field.loadOptions ? asyncState[field.id] : undefined;
    const source = field.loadOptions ? asyncData?.options ?? [] : field.options ?? [];
    const items: ListItem[] = source.map((o) => ({ value: o.value, label: o.label, glyph: o.glyph, selected: filter.values.includes(o.value) }));
    return (
      <SearchList
        key={anchorKey}
  ...
  return (
    ...
    >
      <style>{`
@keyframes fb-flash {
  0% { background-color: rgba(139, 92, 246, 0.22); }
  100% { background-color: transparent; }
}
@media (prefers-reduced-motion: reduce) {
  @keyframes fb-flash { 0%, 100% { background-color: transparent; } }
}
`}</style>
        ...
        {value.map((filter) => {
          ...
          return (
            ...
            >
              ...
              >
                {field?.icon && (
                  <span className="text-zinc-500 dark:text-zinc-400" aria-hidden>{field.icon}</span>
                )}
                {field?.label ?? filter.field}
              </Segment>

              <span aria-hidden className="w-px self-stretch bg-zinc-950/[0.07] dark:bg-white/[0.08]" />
                ...
                {!summary.empty && summary.glyphs.length > 0 && (
                  <span className="flex shrink-0 items-center gap-0.5" aria-hidden>
                    {summary.glyphs.map((g, i) => (
                      <span key={i} className="flex items-center">{g}</span>
                    ))}
                  </span>
                )}
                <span className="max-w-[12rem] truncate font-medium text-zinc-900 dark:text-zinc-50">
                  {summary.empty ? (
                    <span className="font-normal text-zinc-400 dark:text-zinc-500">{summary.text}</span>
                  ) : (
                    summary.text
                  )}
                </span>
