...
public static class SteamService
{
    ...
    // stplug-in lua format (Millennium v2+).
    // File goes to Steam\config\stplug-in\<appId>.lua
    // stplug-in reads it with dofile() and uses the returned table to
    // patch Steam's store/library UI — shows Install instead of Purchase.
    public static string GenerateLua(int appId, string name, List<int>? depotIds = null)
    {
        var safeName = name.Replace("\\", "\\\\").Replace("\"", "\\\"");
        var sb = new StringBuilder();
        sb.AppendLine($"-- stplug-in entry for AppID {appId}");
        ...
        {
            sb.Append("  depots = { ");
            sb.Append(string.Join(", ", depotIds));
            sb.AppendLine(" },");
        }
        else
        {
    ...
    // Remove any existing ACF manifest for this appId across all known library paths.
    public static int RemoveAllAcf(string steamPath, int appId)
    {
        int removed = 0;
        var libs = GetAllLibraryFolders(steamPath);
        foreach (var lib in libs)
        {
            var acfPath = System.IO.Path.Combine(lib.Path, $"appmanifest_{appId}.acf");
            try { if (File.Exists(acfPath)) { File.Delete(acfPath); removed++; } } catch { }
        }
        return removed;
    }

    // Clean re-add via LuaTools:
    //   1. Delete any existing appmanifest ACF (caused PURCHASE button)
    //   2. Write stplug-in lua to config\stplug-in\ (Millennium shows Install)
    // DO NOT write a new ACF — stplug-in handles everything via JS patch.
        ...
        string luaToolsDir, List<int>? depotIds = null)
    {
        ...
        // Step 1 — purge existing ACF manifests that cause PURCHASE
        try
        {
            int removed = RemoveAllAcf(steamPath, appId);
            if (removed > 0)
                msgs.AppendLine($"[CLEAN] Removed {removed} existing manifest(s) for {appId}");
        }
        catch (Exception ex)
        { msgs.AppendLine($"[WARN] ACF cleanup: {ex.Message}"); }

        // Step 2 — write stplug-in lua
        try
        {
        ...
        { msgs.AppendLine("[ERROR] Access denied — run as Administrator."); }
        catch (Exception ex)
        { msgs.AppendLine($"[ERROR] Lua write: {ex.Message}"); }
