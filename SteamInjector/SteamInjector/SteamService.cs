...
public static class SteamService
{
    ...
    {
        // Registry: HKCU first (most common), then HKLM 32/64-bit
        var regKeys = new[]
        {
        ...
        foreach (var (hive, sub) in regKeys)
        {
            try
            {
    ...
    public static List<SteamLibrary> GetAllLibraryFolders(string steamPath)
    {
        ...
        try
        {
            ...
            foreach (Match m in Regex.Matches(text, @"""path""\s+""([^""]+)"""))
                AddLib(libs, m.Groups[1].Value.Replace(@"\\", @"\"));
            foreach (Match m in Regex.Matches(text, @"""[0-9]+""\s+""([a-zA-Z]:[^""]+)"""))
                AddLib(libs, m.Groups[1].Value.Replace(@"\\", @"\"));
        }
        catch { }
    ...
    private static void AddLib(List<SteamLibrary> libs, string root)
    {
        var apps = System.IO.Path.Combine(root, "steamapps");
        if (Directory.Exists(apps) && !libs.Any(l => l.Path.Equals(apps, StringComparison.OrdinalIgnoreCase)))
            libs.Add(new SteamLibrary { Path = apps, Label = $"{System.IO.Path.GetPathRoot(root)} → {apps}" });
    }
    ...
    // ── Manifest / Lua generation ─────────────────────────────────────────────

    public static string GenerateAcf(int appId, string name, int stateFlags = 4,
        string installDir = "", string buildId = "0",
        List<(int depotId, string manifestId)>? depots = null)
    {
    ...
    // Generates a stplug-in / Millennium compatible lua package.
    // File must land in Steam\config\stplug-in\<appId>.lua
    // Millennium loads it via dofile(), returning the table lets stplug-in
    // register the app and patch Steam's UI to show "Install" instead of "Purchase".
    public static string GenerateLua(int appId, string name, List<int>? depotIds = null)
    {
        var safeName = name.Replace("\"", "\\\"").Replace("\\", "\\\\");
        var sb = new StringBuilder();
        sb.AppendLine($"-- stplug-in · AppID {appId} · {safeName}");
        sb.AppendLine($"-- Generated by SteamInjector {DateTime.UtcNow:yyyy-MM-dd}");
        sb.AppendLine();

        // Depot table (optional but improves compatibility)
        sb.AppendLine("local depots = {");
        if (depotIds is { Count: > 0 })
            foreach (var d in depotIds)
                sb.AppendLine($"  {d},");
        sb.AppendLine("}");
        sb.AppendLine();

        // stplug-in Package table — this is what Millennium's stplug-in reads
        sb.AppendLine("local Package = {");
        sb.AppendLine($"  appid       = {appId},");
        sb.AppendLine($"  name        = \"{safeName}\",");
        sb.AppendLine( "  type        = \"Game\",");
        sb.AppendLine( "  dlcs        = {},");
        sb.AppendLine( "  depots      = depots,");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("return Package");
        return sb.ToString();
    }

    // Writes stplug-in lua + ACF manifest together so Millennium + Steam
    // both recognise the game (shows Install, not Purchase).
    public static (bool ok, string msg) InjectViaLuaTools(
        string steamPath, int appId, string name,
        string luaToolsDir, List<int>? depotIds = null)
    {
        var msgs = new System.Text.StringBuilder();
        bool anyOk = false;

        // 1. Write lua to stplug-in folder
        try
        {
            Directory.CreateDirectory(luaToolsDir);
            var luaPath = System.IO.Path.Combine(luaToolsDir, $"{appId}.lua");
            File.WriteAllText(luaPath, GenerateLua(appId, name, depotIds), Encoding.UTF8);
            msgs.AppendLine($"[LUA] Written: {luaPath}");
            anyOk = true;
        }
        catch (UnauthorizedAccessException)
        { msgs.AppendLine($"[ERROR] Access denied writing lua — run as Administrator."); }
        catch (Exception ex)
        { msgs.AppendLine($"[ERROR] Lua: {ex.Message}"); }

        // 2. Write ACF manifest to default steamapps so Steam can track install state
        try
        {
            var appsPath = System.IO.Path.Combine(steamPath, "steamapps");
            if (Directory.Exists(appsPath))
            {
                var acf     = GenerateAcf(appId, name, stateFlags: 4);
                var acfPath = System.IO.Path.Combine(appsPath, $"appmanifest_{appId}.acf");
                File.WriteAllText(acfPath, acf, Encoding.UTF8);
                msgs.AppendLine($"[ACF] Written: {acfPath}");
            }
        }
        catch (Exception ex)
        { msgs.AppendLine($"[WARN] ACF: {ex.Message}"); }

        return (anyOk, msgs.ToString().Trim());
    }

    /// <summary>
    /// Writes appmanifest_APPID.acf (and optionally a Lua config) to steamAppsPath.
    /// Returns (ok, message).
    /// </summary>
    public static (bool ok, string msg) InjectManifest(
        ...
        int stateFlags = 4, string buildId = "0",
        List<(int, string)>? depots = null)
    {
        try
        {
            if (!Directory.Exists(steamAppsPath))
                return (false, $"[ERROR] steamapps path not found: {steamAppsPath}");

            var acf = GenerateAcf(appId, name, stateFlags, buildId: buildId, depots: depots);
            ...
            if (generateLua && !string.IsNullOrEmpty(luaOutputDir))
            {
                ...
                File.WriteAllText(System.IO.Path.Combine(luaOutputDir, $"{appId}.lua"),
                    GenerateLua(appId, name), Encoding.UTF8);
            }

            return (true, $"[OK] Injected: {acfPath}");
        }
        catch (UnauthorizedAccessException)
        {
            return (false, $"[ERROR] Access denied — run as Administrator to write to {steamAppsPath}");
        }
        catch (Exception ex)
        {
