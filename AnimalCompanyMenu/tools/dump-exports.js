/**
 * dump-exports.js — prints the guessed (canonical -> obfuscated) IL2CPP export
 * map for the running game so you can verify it or paste it into
 * src/il2cpp-exports.ts (MANUAL_EXPORT_MAP).
 *
 *   PC   : frida -l tools/dump-exports.js AnimalCompany.exe
 *   Quest: frida -U -l tools/dump-exports.js -n <package>
 *
 * No build step needed – this is plain JavaScript.
 */
const CANONICAL = [
    "il2cpp_init",
    "il2cpp_init_utf16",
...
function main() {
    const mod = Process.findModuleByName("GameAssembly.dll")
        ...
        || Process.findModuleByName("GameAssembly.so")
        || Process.findModuleByName("UnityFramework");
    if (!mod) { console.log("[dump-exports] il2cpp module not found (is the game running / loaded?)"); return; }
    const exports = mod.enumerateExports().filter(e => e.type === "function");
    console.log(`[dump-exports] module ${mod.name} @ ${mod.base}, ${exports.length} function exports`);
    const plain = exports.filter(e => e.name.startsWith("il2cpp_"));
    if (plain.length > 20) {
        console.log(`[dump-exports] exports are NOT obfuscated (${plain.length} il2cpp_* names). Nothing to do.`);
        return;
    }
    const sorted = exports.slice().sort((a, b) => a.address.compare(b.address));
    console.log(`[dump-exports] canonical list has ${CANONICAL.length} names; module has ${sorted.length}.`);
    if (sorted.length !== CANONICAL.length) {
        console.log("[dump-exports] WARNING: counts differ – the mapping below is a guess. Compare with a community map before trusting it.");
    }
    const n = Math.min(sorted.length, CANONICAL.length);
    console.log("\n// paste into MANUAL_EXPORT_MAP in src/il2cpp-exports.ts");
    console.log("export const MANUAL_EXPORT_MAP: Record<string, string> = {");
    for (let i = 0; i < n; i++) {
        console.log(`    ${CANONICAL[i]}: "${sorted[i].name}",   // ${sorted[i].address}`);
    }
    console.log("};");
}
